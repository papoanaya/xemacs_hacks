...
;(defvar delicious-api-host "api.del.icio.us")
;(defvar delicious-api "/v1/")

(defvar delicious-api-host "192.168.10.15/semantic/www")
...
;; (defcustom delicious-host-name "api.del.icio.us"
;;    "The delicious host name."
;;    :version "21.4.1"
;;    :group 'delicious
;;    :type 'string
...
;; (defcustom delicious-api-url "/v1/"
;;    "*The path to the del.ici.ous api.  It should begin and end in a slash."
;;     :version "21.4.1"
;;     :group 'delicious
;;     :type 'string
...
;(setq delicious-api-host delicious-host-name)
;(setq delicious-api delicious-api-url)
...
%C%y-%m-%dT%H:%M:%SZ)."
  (let* ((description (url-hexify-string description))
         ...
         (post-url (format 
                    "posts_add.php?&url=%s&description=%s&tags=%s&extended=%s&dt=%s"
...
(defun delicious-api/tags/rename (old new)
  ...
  (let ((path
         (format "tags_rename.php?&old=%s&new=%s"
                 old new)))
    (delicious-api-request path)))

(defun delicious-api/posts/delete (url)
  "Delete URL from bookmarks."
  (let ((path (format "posts_delete.php?url=%s" url)))
    (delicious-api-request path)))
...
The value returned is a time value."
  (let* ((path (format "posts_update.php"))
