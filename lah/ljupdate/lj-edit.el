...
(if (featurep 'xemacs )
    (progn
      (defun lj-make-button (label beg end &rest properties)  "Make a button from BEG to END in the current buffer.
...
This function is included with calendar for compatability with Emacs."
        (let ((extent  (make-extent beg end))
          ...
          (define-key map [return] (lambda () 
                                     (interactive)
;;                                     (debug)
                                     (let ((entry (car (split-string (thing-at-point 'line) " - " ))))
                                       (lj-edit-post entry))))
          (set-extent-keymap extent map)
          (set-extent-mouse-face extent 'highlight)
          (set-extent-property extent 'button extent)
;;                     (set-extent-face extent 'diary-button) ;; set the properties from the calling function
;;                     (set-extent-properties extent  properties)
          extent))

;; XEmacs change, mimic button.el from Emacs 22
;;;###autoload
      (defun lj-insert-button (label &rest properties)
        "Insert a button with the label LABEL. The remaining arguments form a sequence of PROPERTY VALUE pairs.
         This function is included with calendar for compatability with Emacs."
        (apply #'lj-make-button label (prog1 (point) (insert label))
               (point)
               properties))))
...
(defun lj-insert-entry-into-entry-list (hash n)
  (lexical-let* ((event_string (concat "events_" (number-to-string n)))
    ...
    (if (<= n (string-to-number (gethash "events_count" hash)))
        (progn
          (if (featurep 'xemacs )
              (lj-insert-button (concat event_itemid " - "(gethash event_time hash) " - "
