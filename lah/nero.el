...
;;'iso-2022-8
(defvar nero-default-coding-system 'iso-8859-2
...
(defvar nero-mode-map
  (let ((map (make-keymap)))
    ...
    (if (featurep 'sxemacs)
        (define-key map [(return)] 'nero-follow-link)
      (define-key map (kbd "RET")  'nero-follow-link))

;;    (define-key map (kbd "TAB") 'nero-move-to-next-link)
    (define-key map "\C-cf" 'nero-follow-link)
    (define-key map "\C-ct" 'nero-move-to-next-link)
