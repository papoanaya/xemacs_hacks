...
(defconst org-sxemacs-p (featurep 'sxemacs))
...
(when (or org-xemacs-p org-sxemacs-p)
  (unless (fboundp 'call-process-shell-command)
    (defun call-process-shell-command (command &optional a b c d g)
      (shell-command command)))
  (unless (fboundp 'cancel-timer)
    (defun cancel-timer (itimer)
