...
;  (message url-http-end-of-headers)
;  (message url-http-content-length)
  (message (int-to-string (- nd url-http-end-of-headers) ) )
...
(defun url-http-wait-for-headers-change-function (st nd length)
  ...
  (when (not (bobp))
    (let ((end-of-headers nil)
      ...
      (if (or (looking-at "^verify.*")
              (looking-at "^depth.*"))
          (setq end-of-headers nil)
