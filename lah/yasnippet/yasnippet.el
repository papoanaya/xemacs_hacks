...
;; Overlay compatibility functions
  ...
  (defun yas-overlay-before-string (ovl text &optional face evap)
    "Make overlay OVL display TEXT with face FACE."
    (if (featurep 'xemacs)
        (let ((gl (make-glyph text)))
          (and face (set-glyph-face gl face))
...
;            (add-hook 'after-change-functions 'yas/on-field-xemacs-modification))
...
Do this unless `yas/dont-activate' is truish "
  ...
  (if (featurep 'emacs)
    ...
    (unless (if (functionp yas/dont-activate)
                (funcall yas/dont-activate)
              yas/dont-activate))
    (yas/minor-mode 1)))
  ...
  (catch 'abort
    (let ((errors)
      ...
      (if (featurep 'emacs)
        ...
        (setq yas/minor-mode-map (yas/init-minor-keymap)))
...
(defun yas/on-field-xemacs-modification (beg end length)
  "Clears the field and updates mirrors, conditionally.
...
progress."
  (unless (yas/undo-in-progress)
    ...
    (let*  (
           ...
           (snippet (overlay-get yas/active-field-overlay 'yas/snippet)))
      (cond
            ((>=  beg (marker-position (yas/field-start field)))
               ...
               (let (yas/active-field-overlay)
                 (dolist (field (yas/snippet-fields snippet))
                   (when field
                     ...
                     (setq yas/input-value (read-from-minibuffer (format  "Enter value for field #%d: " (yas/field-number field))))

                     (when (and yas/input-value (> (length yas/input-value) 0)) 
                         (progn
                           (yas/skip-and-clear field)
                           (insert yas/input-value)))))
